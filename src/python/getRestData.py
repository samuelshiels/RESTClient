...
ae['log_file_dir'] = logFilePath = os.path.join(
    str(Path.home()), '.cache', appName)
ae['debug'] = False
...
def __executeCall(restObj: RESTObject, sleep: int) -> str | dict:
    ...
    try:
        ...
        if operation == 'get':
            __debugMessage(['Running REST Call', operation, endpoint,
                           params, headers, get_short_string(payload)])
...
def execute(config, log=False):
    ...
    if config['output'] is not None and config['no_cache'] is not True:
        # 004 - Read File
        returnJSON = __readCache(
            config['output'], config['cache'] or os.getcwd(), config['time'])
    # 005 - Rest Call
    ...
    if returnJSON is not False and config['output'] and runCall:
        if 'errorCode' not in returnJSON:
            errorObj = __writeCache(
                config['output'], returnJSON, config['cache'])
